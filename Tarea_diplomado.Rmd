...
ggplot(D, aes(x=D$MeanWaterTemperature))+
  geom_histogram(color="black", fill="blue", bins = 10)+theme_classic()+theme(text = element_text(size=20,face="bold",colour="black"))+xlab("Temperatura promedio c°")
```
...
```{r}
plot(Case$residuals, pch=20, col = "blue",
...
mortalidad, estos datos, por ser colectados de un zona de fiordos con 5 areas de manejo (simil
a un ´barrio´ en Chile) donde se cultiva salmón del atlantico, donde se vieron afectados por
eventos de mortalidad 87 de 150 centros de cultivo el año 202. Los datos de 150 centros
...
estadístico Prueba de Shapiro-wilks cuyo valor de p es de &gt;0,0001.
...
aceptados datos casi normales, particularmente si en n es &gt;a 30. La distribución a la derecha
