...
```{r}
summarise(N=n(), mean(D), Variance= var(Mortalidad))
```
...
Hipotesis 0: La densidad está asociada al caso (1)
Hipotesis 1: La densidad no esta asociada al caso(1)
...
```{r}
cor(x=D$Density, y=D$Case, method='spearman')
```
Escala Spearman:
...
## Se acepta la hipotesis nula, la correlacion es positiva moderada, y muy posiblemente no lineal.
